        ...
        <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden sm:rounded-lg">
            <div class="flex flex-col justify-center">
                ...
                <div class="flex gap-4 justify-center">
                    <form method="POST" action="{{ route('invite.response', [ "token" => $invitation->token, "action"=> 1 ]) }}">
                        @csrf
                        @method('PATCH')
                    ...
                    <form method="POST" action="{{ route('invite.response', [ "token" => $invitation->token, "action"=> 0 ]) }}">
                        @csrf  
                        @method('PATCH')
